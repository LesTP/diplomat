...
| **Same-provider tier ladder** (OpenAI nano→gpt-5.4-mini→gpt-5.5, and/or Anthropic haiku→sonnet→opus) | ~$3-6 | varies | **Highest-value matrix fix (2026-06-16 review).** Current ladder confounds tier with provider (weak/mid = OpenAI, strong = Anthropic), so "strong-tier" effects can't be separated from "Anthropic" effects. Re-runs the strong tier *within* each provider to disentangle them. |
...
- **Two structural caveats on the whole ablation matrix (added 2026-06-16):** (a) *tier is confounded with provider* — weak/mid are OpenAI, strong is Anthropic, so "strong-tier" effects cannot be separated from "Anthropic" effects without a same-provider ladder; (b) *both sonnet cells are saturated* — WR-β at the floor (0/3 = 0/3, strategic refusal) and jsm1 at the ceiling (3/3 = 3/3) — so sonnet contributes **no usable harness-*lift* evidence**; only the weak/mid cells have headroom to measure lift. Read earlier "sonnet's harness contribution = 0" framings as "unmeasurable on sonnet," not "measured zero."
