...
|---|---|---|---|---|---|---|
| Verdanian Succession (succ3b, 2-way heartland) | strong | claude-sonnet-4-6 | bare | 3 | **2/3** | headroom confirmed; closes the 3-way split (heartland to a claimant, loser + gamma split treasury/defense); alpha won both deals - possible seat-bias, n=2 (Run 21) |
| Verdanian Succession (succ3b, 2-way heartland) | mid | deepseek-chat (V3) | bare | 3 | **0/3** | gamma over-grabs both treasury+defense -> heartland loser starved -> partial-coalition no-deal every game (Run 21) |
...
**Most recent finding (Run 21, 2026-06-26):** `succ3b` (2-outcome-heartland variant of succ-v3; industrial_heartland = Alpha/Beta only, gamma a pure kingmaker) - HOMOGENEOUS baselines, bare, 3 games each, to (a) check deals close and (b) probe seat-bias before the skill ladder. **Result: headroom confirmed - deepseek-chat 0/3 vs claude-sonnet-4-6 2/3.** Failure mechanism is precise: gamma (kingmaker) over-grabs BOTH treasury and defense, starving the heartland loser to exactly BATNA (6) -> no unanimous 3-way deal (scored `partial_coalition_without_coalition_values`). deepseek falls into the over-grab every game; sonnet avoids it 2/3 by giving the heartland loser one asset and gamma the other - the exact brief-validated 3-way split (e.g. heartland->Alpha, treasury->Beta, defense->Gamma = 14/10/12, all clear BATNA). So succ3b is NOT broken and DOES discriminate capability (on the broker-the-3-way-deal axis), unlike succ (focal point) and succ2 (deadlock). Caveats: (i) **possible alpha seat-bias** - with skill held constant (sonnet on all seats) alpha won both closed deals (heartland->Alpha both times); n=2, likely first-mover anchoring; seat rotation (runrotate) needed to confirm/control. (ii) sonnet's 3rd game also hit the gamma over-grab (`deal_below_batna_for_some_faction`). Bonus: live validation that the Phase 47 coalition scoring path correctly flags the partial coalitions. Tooling notes: `verify_dryrun` does not recognize bare-mode call logs (false '0 GEN'); `deepseek-r1` was probed but is unusable (returns chain-of-thought prose, not JSON - needs a CoT-tolerant parser in toolkit). Cost ~$1.3. Next: single-provider tier ladder, seat-rotated, for the capability gradient + alpha seat-bias control. Evidence: `tests/self_play/results/run17_bare_{deepseekchat,claudesonnet46}_succ3b_*.json`.
