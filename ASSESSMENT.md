...
- Status: **Per-game lens + cross-game aggregator DONE** (2026-06-22). `_rank_among_factions()` in `tests/self_play/game_environment.py` emits `faction_ranks` (absolute points, 1=highest, competition ranking for ties) + `ranked_factions`; wired into `score_game()` and rendered in `analysis.py`. Tested in `tests/test_self_play.py::TestRankAmongFactions`. The faction→model map is now persisted to the result JSON as `faction_models` (per-game generator model per faction; override or env-default primary commodity), so rank→model attribution is possible downstream. **Still pending:** only the optional position-rotation harness for scenario-asymmetry control (the per-game lens and the cross-game mean_rank aggregator are both done: tests/self_play/rank_aggregator.py, tested in tests/test_rank_aggregator.py).
