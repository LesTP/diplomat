...
D-66: Phase 51 - paper-hygiene bundle scope (uniform temp all-slots + external index + N-pooling/CIs)
Date: 2026-07-19 | Status: Closed (Phase 51 planned) | Priority: Important
Decision: Phase 51 ("Phase C" in WORK_SEQUENCING section 9) bundles the three
Decision: Phase 52 ("Phase C" in WORK_SEQUENCING section 9) bundles the three PAPER_PLAN section 5.0 hygiene items as ONE autonomous Build phase, each a hermetic, zero-LLM-cost, test-gated slice (contract in ARCH_self_play.md). (1) UNIFORM TEMPERATURE: fix `game_environment._generate_faction_config` to apply the resolved temperature to EVERY provider slot in the emitted per-faction config, not just the generator slot (the analyst/adversarial/extractor/reconciler/scorer slots currently always run at the 0.7 default); the gpt-5.x/o-series `temperature=1` exception STAYS (toolkit omits the param) but is made explicit by recording which slots are temperature-exempt; bring ablation.sh + ablation_jsm1.sh to `TEMPERATURE`-env parity with ablation_multi.sh (additive shell, not test-gated). (2) EXTERNAL INDEX: new Diplomat-side module `tests/self_play/external_index.py` (NOT the shared toolkit tree - paper-only reference), an `EXTERNAL_INDEX` dict + `ExternalIndex` frozen dataclass (lmarena_elo, mmlu, source, as_of) + `lookup()` mirroring `toolkit/cost_accountant` `DEFAULT_PRICING` shape and `normalize_model_name` resolution; tests lock structure/presence/range only, NOT value correctness (operator confirms the numbers - supervised). (3) N-POOLING + CIs: reinterpret the vague "confirm N-scaling dispatcher" as a real Build - lock the rank_aggregator N-pooling contract with synthetic-fixture tests AND add a DETERMINISTICALLY SEEDED `bootstrap_ci()` helper (stdlib `random`, fixed seed), which is the actual section 5.0 gap.
Rationale: These three gate the section 5.7 construct-validity analysis (needs the external index) and remove the gpt-5.5 temperature confound from every future cell BEFORE the expensive supervised unified campaign (Phase D) spends anything - so they are the cheapest, lowest-risk, fully-loopable first paper build. Pre-deciding the architecture (all-slots override; Diplomat-side index; seeded bootstrap) keeps the phase a clean Build with zero open decisions for the loop, matching the D-61/D-64 pattern. Queued as phase 51 - the CONTIGUOUS next phase after 50 - so the operator Telegram control surface can dispatch it as the current phase; the i2c bot runs the current/contiguous phase and cannot dispatch a discontinuous one (phase 52 while 51 is unrun). The offering foundation is therefore renumbered to phase 52 (ARCH_flow.md section Phase 52 / papers/OFFERING.md) and deferred per operator direction to focus on the paper. (Supersedes the initial authoring, which queued hygiene as 52 and left the offering at 51.)
Revisit if: the temperature all-slots fix surfaces a support-module that MUST stay at a different temperature (would become a per-slot policy, not a single uniform value); or if bootstrap CIs need to move into a shared analysis module consumed beyond rank_aggregator.
D-67: Phase 52 = mechanism classifier (machinery autonomous, validation supervised); offering shelved to Phase 0
Date: 2026-07-19 | Status: Closed (Phase 52 planned) | Priority: Important
Decision: Phase 52 is the mechanism classifier (PAPER_PLAN section 4.1), NOT the client-agent-ingress offering. The offering is shelved off-sequence to Phase 0 (docs only - it was never registered in .state) so the i2c bot dispatches phase 52 = classifier as the contiguous next after 51; renumber Phase 0 back to reactivate. Contract in ARCH_mechanism_classifier.md, mirroring src/modules/edit_classifier over toolkit.structured_llm: a build_mechanism_classifier factory + MechanismClassifier calling structured_call with a 5-boolean schema (Q1 substantive engagement, Q2 subset convergence, Q3 explicit BATNA preference, Q4 identity/coherence failure, Q5 full agreement) + a pure derive_label rule -> {closed, strategic_refusal, near_miss, incoherence, breakdown}; judges from results.transcript + final round_responses ONLY (scores.* only for validation cross-checks); fixed independent judge (_MECHANISM_JUDGE const, not a contestant); config/prompts/mechanism_classifier.txt; tools/classify_mechanisms.py batch CLI; stdlib per-question Cohen kappa (no sklearn). SPLIT - AUTONOMOUS loopable Build slice: schema + deterministic rule + classifier + factory + CLI + kappa function + DryRunLLMClient mechanism route + an INITIAL prompt + fake-backed hermetic tests. ASSISTED supervised follow-on (loop STOPS here, per the D-62/Phase-48 boundary): (A) author the hand-label seed set mechanism_seed_labels.json (none exists yet - only prose in PAPER_PLAN/TUNING_LOG), (B) tune the judge prompt, (C) live per-question kappa validation with real keys+cost + a ship threshold, (D) pick the independent judge model.
Rationale: The classifier is the linchpin of the paper co-primary harness-lift contribution (turns the three-mechanism taxonomy from anecdote into data) and is reused by the offering detection tier + tactic-library calibration (OPEN_ITEMS section 5), so it leads the paper thread now that phase 51 hygiene has landed. Mirroring edit_classifier keeps it low-risk. Splitting machinery (objective, test-gated -> autonomous) from validation (human-authored ground truth + judge-quality judgment + live cost -> supervised) matches the GOVERNANCE evaluability rule and the D-62 precedent. Shelving the offering to Phase 0 keeps the sequence contiguous for the operator Telegram control surface (which cannot dispatch a discontinuous phase) while preserving the full offering spec for later.
Revisit if: live kappa is unacceptable for a question (prompt-tune or split that question further); or the offering thread resumes (renumber Phase 0 back into the sequence).
