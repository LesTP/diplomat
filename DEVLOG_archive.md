...
## Archived 2026-06-11 — Phase 36 scenario builder improvements + Phase 37 pareto_outcome_diversity metric
...
Tests: 440 passed, 1 skipped (up from 435; +5 new: low-diversity, high-diversity, mixed, default-zero-no-constraint, ScenarioSpec field validation)
Contract changes: none (additive only — new metric field + computation, no existing interfaces modified)

**What was built:**
- `pareto_outcome_diversity` computation added to `compute_fitness()` in `src/tools/scenario_fitness.py`: counts distinct faction winners across the Pareto frontier, normalizes by `min(frontier_size, n_factions)`. A `0.0` target is treated as "no constraint."
- `pareto_outcome_diversity: float = 0.0` field added to `ScenarioSpec` in `src/tools/scenario_spec.py` with `[0.0, 1.0]` validation.
- `joint_space_mission_v1/spec.json` updated with `pareto_outcome_diversity: 0.66`.
- `CLI_REFERENCE.md` updated with structured cross-reference block explaining both metrics side by side.
- Full verify passed: VERIFY PASSED.

**DEVLOG learning review:** No new trial-and-error patterns in this phase. The metric naming confusion (`pareto_distribution_spread` ≠ inter-deal diversity) was the triggering observation and is already captured in DEVPLAN Gotchas from Phase 36.5. Phase 37 was a clean 7-step build.
...
## Recovered rationale 2026-06-23 — Conversation-model evolution deprioritization (from the retired NEXT_STEPS §3)

Preserved during the 2026-06-23 NEXT_STEPS overhaul (see active `DEVLOG.md`
2026-06-23 operator-supervised entry). The deprioritization *rationale* below
lived only in the old NEXT_STEPS §3 banner + Change History; it is not the
staged design space (that is in `ARCH_conversation_model.md`) and was at risk of
loss when those sections were dropped.

**Decision (2026-06-02):** Stage 2a (K-pass open+react rounds) was removed from
Tier 1. **Why:** on review, the pressure mechanisms (§2) work fine with extra
rounds on the current Stage 1 sealed-bid model — the "pass" vs "round"
distinction doesn't produce meaningfully different agent behavior, and K=2
open+react is functionally ≈ running 2× rounds with adjusted endgame markers.
The full conversation-model migration is interesting but was not blocking
anything. **When to revisit:** if Stage-1 runs show agents talking past each
other in ways more rounds can't fix (not seen in Runs 1–10); if a real-game
moderator wants within-round reaction dynamics; or if async timing-as-strategy
becomes a research goal. Under D-56 (benchmark direction) this is held in the
NEXT_STEPS "Deferred (icebox)" list — pursue when a benchmark scenario class
demands within-round reactivity or timing-as-strategy.

## Archived 2026-06-23 — Phases 28–34 + 39 + pre-Phase-43 operator entries (moved from active DEVLOG)
<!-- Bulk-archived 2026-06-23: the per-close archival step had been skipped across several phases, so the active DEVLOG had grown to ~830 lines. Phase 37 close was dropped here (already present above). -->
...
## Phase 47 close (2026-06-25) — full entry
...
## Archived 2026-06-27 — Phase 48 close + Phase 49 (operator-supervised sessions + Phase 49 close)

Moved during CLOSE action for Phase 49. Active DEVLOG cleared for next phase.
...
**Governance updates:** D-64 closed. DEVPLAN Phase 49 condensed (see Current Status). Entries archived (this file).
