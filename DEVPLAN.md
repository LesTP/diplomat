...
## Current Status
- **Phase 49 complete (2026-06-27):** Per-run cost metadata (`metadata.cost_usd`) + backfill tool — 637 tests, D-64 closed; see `DEVLOG_archive.md` "Phase 49 close".
...
## Phase 49: Per-run cost capture + approximate backfill — Complete

Closed 2026-06-27. `metadata: {cost_usd, cost_source, n_llm_calls}` block added to self-play result JSONs — live runs write `cost_source: "metered"` from `accountant.session_total`; dry-run path writes `"dry_run"`. `FakeCostAccountant.session_total` added to `tests/helpers/factories.py`. `tools/backfill_cost.py` re-estimates cost from `llm_call_log` token counts and backfills 108 historical results at `cost_source: "estimated_from_log"` (idempotent; skips `"metered"` runs). 17 new tests. D-64 closed. 637 passed, 1 skipped. See `DEVLOG_archive.md` "Phase 49 close".
