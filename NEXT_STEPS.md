...
| **§3.5 rank-among-factions scoring lens** | `[C]` | ✅ per-game DONE (2026-06-22) | ASSESSMENT §3.5 | **Per-game lens + faction→model persistence shipped:** `_rank_among_factions` (`faction_ranks` by absolute points, competition ties) + render + tests; `faction_models` map now persisted to result JSON. **DONE (2026-06-22):** position-rotation harness added (tests/self_play/position_rotation.py + ablation_multi.sh runrotate/rotateplan; tests/test_position_rotation.py). Section 3.5 is fully implemented (lens + faction_models + aggregator + rotation). |
| **Mixed-model dispatcher extension** | `[C][X]` | per-faction DONE (2026-06-22) | (§11 follow-up) | `tools/ablation_multi.sh` now supports heterogeneous populations via `runmix 'faction=MODEL,...' SCENARIO MODE N` (+ `probemix`); per-faction models are recorded in the result `faction_models` map for the rank aggregator. Position-rotation control still pending. Unblocks §11.b Path A calibration. |
