...
| **§3.5 rank-among-factions scoring lens** | `[C]` | ✅ per-game DONE (2026-06-22) | ASSESSMENT §3.5 | **Per-game lens + faction→model persistence shipped:** `_rank_among_factions` (`faction_ranks` by absolute points, competition ties) + render + tests; `faction_models` map now persisted to result JSON. **Remaining:** cross-game `mean_rank(model)` aggregator (join ranks × faction_models across results) + optional position-rotation harness. |
