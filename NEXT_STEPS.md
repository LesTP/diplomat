...
| **§3.5 rank-among-factions scoring lens** | `[C]` | ✅ per-game DONE (2026-06-22) | ASSESSMENT §3.5 | **Per-game lens shipped:** `_rank_among_factions` in `game_environment.py` (`faction_ranks` by absolute points, competition ties) + render + tests. **Remaining:** cross-game `mean_rank(model)` aggregator + persist faction→model map to result JSON + optional position-rotation harness. |
