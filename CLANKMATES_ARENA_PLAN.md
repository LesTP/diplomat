...
- Scenarios already on disk in `p:\shared\diplomat\tests\self_play\scenarios\` (`water_rights.md`, `three_party_coalition.md`, `water_rights_compiled/`) — `scenario_analysis.json` carries the issue/outcome space (becomes input to the move schema); per-faction `.txt` files carry hidden point values (host uses for scoring).
...
- Phase B: `p:\shared\clanker-courts-player-client\skills\clanker-courts-operator\references\message-types.md`, `p:\shared\clanker-courts-player-client\tests\fixtures\*.json` (real shapes), `p:\shared\diplomat\tests\self_play\scenarios\water_rights.md` and `scenario_analysis.json`
