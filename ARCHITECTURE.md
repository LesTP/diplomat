...
| 25 | Client-Agent Ingress + 1:1 Harness | Offering foundation (T1+T2): a `ClientAgentGenerator` runs an external agent-under-test from **[system prompt] + [transcript]** (bypassing our scaffolding) against a simulated-customer persona in an **asymmetric 2-party** `GameEnvironment` mode; bounded bilateral loop (deal / walk-away / turn-cap); 2-party-aware scoring (coalition analytics off at N=2). Prompt path only; live-API path deferred. Contract + acceptance tests in [`ARCH_flow.md`](ARCH_flow.md) §"Phase 52". | Phase 52 — deferred (offering; see `papers/OFFERING.md`) |
