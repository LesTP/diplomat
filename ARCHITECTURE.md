...
- **Scenario Authoring ↔ structured_call:** the subsystem (`src/scenario_authoring/`, see [`ARCH_scenario_authoring.md`](ARCH_scenario_authoring.md)) routes all LLM calls through `toolkit.structured_llm.structured_call` at the `commodity` tier. Two LLM-consuming entry points: `scenario_compiler.analyze_scenario` (narrative → structured analysis) and `scenario_compiler.fill_narrative` (overlay prose onto builder-emitted stubs). Three pure-Python modules (`scenario_builder`, `scenario_fitness`, `scenario_spec`) consume no LLM. `verify_scenario_optimum` is a pure CLI + library that reads an analysis and reports its payoff structure. No runtime pipeline dependency — the subsystem produces files; `GameEnvironment.score_game` reads them.
