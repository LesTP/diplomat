...
- **Review Gate ↔ Transport:** tight — `OperatorReviewGate` consumes the pipeline's `Transport` for coaching-channel I/O. No direct `toolkit/telegram_client` dependency. The orchestrator factory passes the already-built transport module when constructing the review gate.
