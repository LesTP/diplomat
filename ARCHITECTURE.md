...
| 24 | Paper Hygiene (uniform temp · external index · N-pooling + CIs) | Paper-1 §5.0 hygiene bundle (Phase 51), pure-code/data + hermetic: (1) uniform sampling temperature across **all** provider slots in self-play cells (fix `_generate_faction_config` generator-only override; gpt-5.x=1 documented exception); (2) new Diplomat-side `external_index` lookup (roster → LMArena Elo + MMLU) mirroring `cost_accountant` `DEFAULT_PRICING` / `normalize_model_name`; (3) lock N-pooling with tests + a deterministic seeded bootstrap-CI helper for the aggregator. Contract + acceptance tests in [`ARCH_self_play.md`](ARCH_self_play.md) §"Phase 51". D-66. | Phase 51 — planned |
