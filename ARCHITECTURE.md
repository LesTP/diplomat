...
| 17 | Scenario Builder | Constraint-driven reverse scenario generator. Operator writes a `ScenarioSpec`; tool searches scoring-table space via hill-climb and emits `scenario_analysis.json` + per-faction `.txt` personas. Not a pipeline module — design tool only. | Phase 35 complete |

## Testing Status

| Layer | Status |
|-------|--------|
| Unit and regression tests | Complete — 426 tests after Phase 35 (added `ScenarioSpec`, fitness, builder, and CLI unit tests; 1 replay test is flaky in full-suite ordering, passes in isolation) |
