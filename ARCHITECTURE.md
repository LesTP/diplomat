...
| 23 | Event Store Result-Persistence | Per-agent `event_log` field added to self-play result JSON (Phase 50). `collect_results()` queries each agent's event store before teardown and serializes all events as `{event_id, round_number, timestamp, sender_faction, channel, recipient, content, telegram_msg_id}`. Empty factions emit `event_log: []`. 2 acceptance tests. D-65 closed. | Phase 50 complete |
| 24 | Paper Hygiene (uniform temp · external index · N-pooling + CIs) | Paper-1 §5.0 hygiene bundle (Phase 51), pure-code/data + hermetic: (1) uniform sampling temperature across **all** provider slots in self-play cells (fix `_generate_faction_config` generator-only override; gpt-5.x=1 documented exception; `ablation.sh` + `ablation_jsm1.sh` now mirror `ablation_multi.sh`'s `TEMPERATURE` env override); (2) new Diplomat-side `external_index` lookup (roster → LMArena Elo + MMLU) mirroring `cost_accountant` `DEFAULT_PRICING` / `normalize_model_name`; (3) lock N-pooling with tests + a deterministic seeded bootstrap-CI helper for the aggregator. Contract + acceptance tests in [`ARCH_self_play.md`](ARCH_self_play.md) §"Phase 51". D-66. | Phase 51 complete |
| 25 | Mechanism Classifier | LLM-as-judge (`src/modules/mechanism_classifier/`, factory mirroring `edit_classifier`) labeling each self-play game by failure mechanism via 5 binary questions → deterministic rule → `{closed, strategic_refusal, near_miss, incoherence, breakdown}` (Phase 52). Judges from `transcript` + final `round_responses` only; fixed independent judge (not a contestant); per-question Cohen's κ (stdlib) vs a hand-labeled seed set. Autonomous slice = machinery + initial prompt + fake-backed tests; seed-labels + live κ validation are a supervised follow-on. Contract + acceptance tests in [`ARCH_mechanism_classifier.md`](ARCH_mechanism_classifier.md) §"Phase 52". D-67. | Phase 52 — planned |
| 0 | Client-Agent Ingress + 1:1 Harness | **SHELVED (offering; renumber to reactivate).** Offering foundation (T1+T2): a `ClientAgentGenerator` runs an external agent-under-test from **[system prompt] + [transcript]** (bypassing our scaffolding) against a simulated-customer persona in an **asymmetric 2-party** `GameEnvironment` mode; bounded bilateral loop (deal / walk-away / turn-cap); 2-party-aware scoring (coalition analytics off at N=2). Prompt path only; live-API path deferred. Contract + acceptance tests in [`ARCH_flow.md`](ARCH_flow.md) §"Phase 0 (shelved)". | Phase 0 — shelved (offering; see `papers/OFFERING.md`; D-67) |
...
- **Scenario Authoring ↔ structured_call:** the subsystem (`src/scenario_authoring/`, see [`ARCH_scenario_authoring.md`](ARCH_scenario_authoring.md)) routes all LLM calls through `toolkit.structured_llm.structured_call` at the `commodity` tier. Three LLM-consuming entry points: `scenario_compiler.analyze_scenario` (narrative → structured analysis), `scenario_compiler.fill_narrative` (overlay prose onto builder-emitted stubs), and `scenario_narrative.reskin_scenario` (themed relabel + prose via bijective relabel_map). Pure-Python modules (`scenario_builder`, `scenario_fitness`, `scenario_spec`) consume no LLM. `verify_scenario_optimum` is a pure CLI + library that reads an analysis and reports its payoff structure. No runtime pipeline dependency — the subsystem produces files; `GameEnvironment.score_game` reads them.
